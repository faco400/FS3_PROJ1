...
void write_uart_get(int filestream, unsigned char code) {
  unsigned char package[7] = {0x01, 0x23, code, 0x05,
                              0x05, 0x00, 0x00};
...
void write_uart_send_REF(int filestream, float reference_signal) {
  unsigned char package[7] = {0x01, 0x16, SEND_REFERENCE_SIGNAL, 0x05,
                              0x05, 0x00, 0x00};
  unsigned char msg[13];
  memcpy(msg, &package, 7);
  memcpy(&msg[7], &reference_signal, 4);
  short crc = calcula_CRC(msg, 11);
  memcpy(&msg[11], &crc, 2);
  int count = write(filestream, &msg[0], 13);
  if (count < 0) {
...
void turn_off_system(int filestream, int on_off) {
  unsigned char package[7] = {0x01, 0x16,  SEND_SYS_STATE, 0x05,
                              0x05, 0x00, 0x00};
  unsigned char msg[10];
  memcpy(msg, &package, 7);
  memcpy(&msg[7], &on_off, 1);
  short crc = calcula_CRC(msg, 11);
  memcpy(&msg[8], &crc, 2);
  int count = write(filestream, &msg[0], 10);
  if(count < 0){
    printf("Ocorreu um erro na comunicação com o UART\n");
  }
  return count;
